  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        // กรองโพสต์ที่ตรงกับคำค้นหาหรือรายละเอียดที่ผู้ใช้ป้อนมา
        const searchValue = search ? search.toLowerCase() : "";
        const filteredGames = postsData.filter((post) =>
          post.name_games.toLowerCase().includes(searchValue)
        );

        // ตรวจสอบว่าเรากรองข้อมูลได้ถูกต้องหรือไม่
        console.log("Filtered Games Data:", filteredGames);

        const postsWithParticipants = filteredGames.map((post) => {
