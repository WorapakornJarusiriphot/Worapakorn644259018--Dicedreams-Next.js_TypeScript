...
const AccountDetailsForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        const userResponse = await axios.get(`https://dicedreams-backend-deploy-to-render.onrender.com/api/users/${decodedToken.users_id}`, {
          headers: {
        ...
        if (decodedToken.store_id) {
          const storeResponse = await axios.get(`https://dicedreams-backend-deploy-to-render.onrender.com/api/store/${decodedToken.store_id}`, {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            }
          });
