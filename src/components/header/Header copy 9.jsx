...
function Header() {
  ...
  // เพิ่ม state สำหรับ previewUrl เพื่อจัดเก็บ URL รูปภาพสำหรับผู้ใช้
  const [previewUrl, setPreviewUrl] = useState(null); // เริ่มต้นเป็น null
  const [previewStoreUrl, setPreviewStoreUrl] = useState(null); // สำหรับร้านค้า

  // ฟังก์ชันสำหรับการเปลี่ยนรูปภาพผู้ใช้
  const handleUserImageChange = (event) => {
    const file = event.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setPreviewUrl(reader.result); // กำหนดค่า previewUrl เมื่ออ่านไฟล์เสร็จสิ้น
      };
      reader.readAsDataURL(file);
    }
  };

  // ฟังก์ชันสำหรับการเปลี่ยนรูปร้านค้า
  const handleStoreImageChange = (event) => {
    const file = event.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setPreviewStoreUrl(reader.result); // กำหนดค่า previewStoreUrl เมื่ออ่านไฟล์เสร็จสิ้น
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  // เพิ่ม state สำหรับเก็บข้อมูลร้านค้า
  const [store, setStore] = useState({
    store_image: "",
    name_store: "",
    // ข้อมูลอื่น ๆ ของร้านค้า
  });

  // ฟังก์ชันสำหรับดึงข้อมูลร้านค้าจาก API
  const fetchStoreProfile = async (storeId, accessToken) => {
    try {
      ...
      if (response.status === 200) {
        setStore(response.data); // เก็บข้อมูลร้านค้าใน state
      } else {
  ...
  // ดึงข้อมูล store จาก store_id ใน JWT
  useEffect(() => {
    const accessToken = localStorage.getItem("access_token");
    if (accessToken) {
      const decoded = jwtDecode(accessToken); // ไม่มีการใช้ TypeScript, ไม่ต้องมี Type annotations
      if (decoded.store_id) {
        fetchStoreProfile(decoded.store_id, accessToken);
      }
    }
  }, []);
          ...
          {username ? (
            <>
              ...
              <Tooltip title="บัญชี">
                <Box sx={{ zIndex: 1400 }}>
                  {user.userType === "store" ? (
                    // ถ้าเป็นเจ้าของร้านค้าให้โชว์รูปร้านค้า (store_image)
                    <Avatar
                      src={previewStoreUrl || store.store_image}
                      alt="รูปร้านค้า"
                      sx={{
                        marginLeft: "10px",
                        height: "80px",
                    ...
                    // ถ้าไม่ใช่เจ้าของร้านค้าให้โชว์รูปผู้ใช้ (user_image)
                    <Avatar
