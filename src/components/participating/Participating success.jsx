    ...
    async function fetchData() {
      ...
      try {
        ...
        const postPromises = myParticipations.map(async (participation) => {
          ...
          const userResponse = await fetch(`https://dicedreams-backend-deploy-to-render.onrender.com/api/users/${post.users_id}`, {
            headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }
          });
          ...
          return {
            ...post,
            participants: myParticipations.filter(p => p.post_games_id === post.post_games_id).length,
