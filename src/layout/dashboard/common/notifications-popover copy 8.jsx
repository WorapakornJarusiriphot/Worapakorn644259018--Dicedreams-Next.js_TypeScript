...
export default function NotificationsPopover() {
  ...
  // ฟังก์ชันการดึงข้อมูลการแจ้งเตือนที่แก้ไข
  const fetchNotifications = async (page = 1, limit = 10) => {
    ...
    try {
      const response = await fetch(
        `https://dicedreams-backend-deploy-to-render.onrender.com/api/notification?page=${page}&limit=${limit}`,
        {
          headers: {
      ...
      if (data.messages && Array.isArray(data.messages)) {
        // ตรวจสอบข้อมูลใหม่กับข้อมูลเดิมเพื่อไม่ให้ซ้ำกัน
        setNotifications((prevNotifications) => {
          const newNotifications = data.messages.filter(
            (newNotification) =>
              !prevNotifications.some(
                (oldNotification) =>
                  oldNotification.notification_id ===
                  newNotification.notification_id
              )
          );
          return [
            ...prevNotifications, // ข้อมูลเดิม
            ...newNotifications, // ข้อมูลใหม่ที่ไม่ซ้ำ
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      fetchNotifications(); // ดึงข้อมูลซ้ำทุก 5 วินาที แต่ข้อมูลเก่าจะไม่ซ้ำ
    }, 5000);

    return () => clearInterval(intervalId); // ล้าง interval เมื่อ component ถูกถอดออก
  }, []);

  // // ฟังก์ชันสำหรับโหลดการแจ้งเตือนหน้าใหม่
  // const loadMoreNotifications = () => {
  //   const nextPage = Math.ceil(notifications.length / 10) + 1; // หาหน้าถัดไป
  //   fetchNotifications(nextPage);
  // };

  useEffect(() => {
    fetchNotifications(); // ดึงข้อมูลหน้าแรก
  }, []);
            ...
            >
              {Array.isArray(notifications) &&
                notifications
                  .filter(
                    (notification) =>
                      !filterNewNotifications(notifications).some(
                        (newNotification) =>
                          newNotification.notification_id ===
        ...
        <Scrollbar sx={{ height: { xs: 340, sm: "auto" } }}>
            ...
            {Array.isArray(notifications) &&
              notifications.map((notification) => (
                <NotificationItem
                  ...
                  notification={notification}
                  onMarkAsRead={handleMarkAsRead}
                  onClick={(event) => handleButtonClick(event, notification)}
                />
              ))}
          </List>
          ...
          {notifications.length > 0 && (
            <Box sx={{ textAlign: "center", py: 2 }}>
              <Button onClick={loadMoreNotifications}>โหลดเพิ่มเติม</Button>
            </Box>
          )}
        </Scrollbar>
      </Popover>
