...
function PostGameDetail() {
  ...
  const joinGameClick = async () => {
    const accessToken = localStorage.getItem("access_token");
    const decoded = jwtDecode(accessToken);
    const currentUserId = decoded.users_id;
    const gameId = router.query.id;
      ...
      const response = await fetch(
        "https://dicedreams-backend-deploy-to-render.onrender.com/api/participate",
        {
          ...
          body: JSON.stringify({
            ...
            participant_apply_datetime: new Date().toISOString(),
  ...
  const fetchPostGame = async (gameId, accessToken) => {
    try {
      ...
      const response = await fetch(
        `https://dicedreams-backend-deploy-to-render.onrender.com/api/post_games/${gameId}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
  ...
  useEffect(() => {
    const fetchPostData = async () => {
      ...
      try {
        const response = await fetch(
          `https://dicedreams-backend-deploy-to-render.onrender.com/api/posts/${id}`
        );
          ...
          >
            ...
            >
              ...
              {participants.map((participant, index) => (
                  <Box key={index} textAlign="center" sx={{ margin: 1 }}>
                    <Avatar
                      src={participant.avatar || profilePictureUrl}
