...
function PostGameDetail() {
  ...
  const handleEditChat = (event, chat) => {
    ...
    try {
      ...
      // ตั้งค่า chat_id ที่ต้องการแก้ไข และตั้งข้อความที่จะแสดงใน TextField
      setEditingChatId(chat.chat_id); // ตั้งค่า chat_id ที่จะทำการแก้ไข
      setEditedMessage(chat.message); // กำหนดข้อความที่ต้องการแก้ไข
    } catch (error) {
      console.error("Error decoding token:", error);
      setSnackbarMessage("Token ไม่ถูกต้อง กรุณาเข้าสู่ระบบใหม่");
      setSnackbarSeverity("error");
      setOpenSnackbar(true);
      setTimeout(() => {
        router.push("/sign-in");
    ...
    try {
      const response = await fetch(
        `https://dicedreams-backend-deploy-to-render.onrender.com/api/chat/${chat.chat_id}`,
        {
          ...
          body: JSON.stringify({ message: editedMessage }), // ส่งข้อความที่แก้ไขแล้ว
        }
      );

      if (response.ok) {
        // เมื่อบันทึกสำเร็จให้รีเซ็ตสถานะและรีเฟรชข้อมูล
        setEditingChatId(null); // ยกเลิกโหมดแก้ไข
        window.location.reload(); // โหลดข้อมูลใหม่เพื่อแสดงการแก้ไข
      } else {
