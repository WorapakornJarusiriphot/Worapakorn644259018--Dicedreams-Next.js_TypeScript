...
function PostGameDetail() {
  ...
  const handleEditChat = async (event) => {
    ...
    // ตรวจสอบว่ามีโพสต์ที่เลือกหรือไม่
    if (!selectedPost) {
    ...
    // ตรวจสอบว่า accessToken มีอยู่หรือไม่
    ...
    try {
      ...
      const isTokenExpired = (token) => {
        ...
        const currentTime = Date.now() / 1000; // แปลงเวลาเป็นวินาที
        return decodedToken.exp < currentTime;
      };

      // ตรวจสอบว่า Token หมดอายุหรือไม่
      if (isTokenExpired(accessToken)) {
        setSnackbarMessage("Token หมดอายุ กรุณาเข้าสู่ระบบใหม่");
        setSnackbarSeverity("error");
        setOpenSnackbar(true);
        setTimeout(() => {
          router.push("/sign-in");
      ...
      // กำหนดค่า chat_id และข้อความของโพสต์ที่ต้องการแก้ไข
      setEditingChatId(selectedPost.chat_id); // ใช้ selectedPost ในการตั้งค่า chat_id
      setEditedMessage(selectedPost.message); // ใช้ selectedPost ในการตั้งค่าข้อความที่จะถูกแก้ไข
    } catch (error) {
    ...
    if (selectedPost) {
      try {
        ...
        if (response.status === 201) {
          ...
          // อัปเดตสถานะการเข้าร่วมใน items
          setItems((prevItems) =>
            prevItems.map((item) =>
          ...
          // ตั้งเวลาเพื่อรีเฟรชหน้าจอหลังจากแสดงข้อความสำเร็จ
          setTimeout(() => {
  ...
  const handleChatSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrorMessage("ขออภัย ขณะนี้เซิร์ฟเวอร์มีปัญหาหรือตอนนี้มีคนใช้งานเยอะเกินไป กรุณาลองส่งใหม่อีกครั้ง");
      } else {
      ...
      if (error.message.includes("500")) {
        setErrorMessage("ขออภัย ขณะนี้เซิร์ฟเวอร์มีปัญหาหรือมีคนใช้งานเยอะเกินไป กรุณาลองส่งใหม่อีกครั้ง");
      } else {
