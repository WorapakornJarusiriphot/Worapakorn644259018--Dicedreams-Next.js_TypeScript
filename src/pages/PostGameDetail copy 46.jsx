...
function PostGameDetail() {
  ...
  const fetchChatMessages = async (gameId, accessToken) => {
    try {
      setLoading(true);

      const response = await fetch(
        `https://dicedreams-backend-deploy-to-render.onrender.com/api/chat/post/${gameId}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
  ...
  useEffect(() => {
    ...
    if (accessToken && router.isReady) {
      ...
      if (gameId) {
        ...
        fetchChatMessages(gameId, accessToken); // Fetch chat messages
      } else {
              ...
              {chatMessages.map((chat) => (
                ...
                >
                  ...
                  <Box>
                    ...
                    <Typography variant="body2" color={"#000000"}>
                      {new Date(chat.datetime_chat).toLocaleString("th-TH")}
                    </Typography>
                  </Box>
                </Box>
              ))}
