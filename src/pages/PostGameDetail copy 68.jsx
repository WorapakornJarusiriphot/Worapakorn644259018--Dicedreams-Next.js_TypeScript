  ...
  const fetchChatMessages = async (postId) => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        data.forEach(chat => {
          const date = new Date(chat.datetime_chat);
          if (!isNaN(date.getTime())) {
            chat.datetime_chat = date; // แปลงค่า datetime_chat ให้เป็น Date
          } else {
            console.error("Invalid date format for chat message:", chat.datetime_chat);
            chat.datetime_chat = null;
          }
        });
        setChatMessages(data.filter(chat => chat.datetime_chat !== null).sort((a, b) => a.datetime_chat - b.datetime_chat)); // เรียงลำดับข้อความแชท
      } else {
    ...
    <>
      ...
      <ThemeProvider theme={darkTheme}>
        <Container maxWidth="lg" sx={{ marginTop: 10 }}>
          ...
          {chatMessages
            .sort((a, b) => a.datetime_chat - b.datetime_chat) // เรียงลำดับจากอดีตถึงปัจจุบัน
            .map((chat) => (
              <Paper
                key={chat.chat_id}
                elevation={3}
