...
import { Grid, Checkbox, FormControlLabel, RadioGroup, Radio } from "@mui/material";
...
function PostGameDetail() {
  ...
  const fetchOwnerProfile = async (userId) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("Owner Profile Data:", data); // Log owner profile data
        setOwnerProfile({
  ...
  const fetchPostGame = async (gameId, accessToken) => {
    try {
      ...
      const data = await response.json();
      console.log("Post Data:", data); // Log post data
      ...
      if (data.users_id) {
        fetchOwnerProfile(data.users_id);
      }
    } catch (error) {
  ...
  console.log("Owner Profile State:", ownerProfile); // Log owner profile state
